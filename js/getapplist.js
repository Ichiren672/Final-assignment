...
function get() {
    ...
    let dispnum = 0;
    list.innerHTML="";
    maxpage = Math.ceil(filtered.length/AVIABLEDISPNUM)-1;
    ...
    num.forEach(fordisp => {
        ...
        if (filtered.length === 0) {
            ...
            baseText =
            `${filtered.length}件中 ${currentpage*AVIABLEDISPNUM+1} - ${currentpage*AVIABLEDISPNUM+dispnum}件表示`;
        }
        if (isSearching) {
            fordisp.innerHTML = `${baseText} <span style="color:red;">（検索中）</span>`;
        } else {
            fordisp.textContent = baseText;
        }
    });
    if (currentpage!=0) {
...
function search() {
    ...
    // 検索
    isSearching = true;
    filtered = APPLISTS.filter(item => {
        // 名前検索
        let nameMatch = item.name.includes(keyword);
        // 説明検索
        let descMatch = item.description.includes(keyword);
        // タグ検索
        let tagMatch = false;
        if (item.tag.main.some(t => t.includes(keyword)) || item.tag.code.some(t => t.includes(keyword)) ||
        item.tag.environment.some(t => t.includes(keyword)) || item.tag.other.some(t => t.includes(keyword)))
            tagMatch = true;
